...
object Converter extends ZIOAppDefault:
  val parserWithDefaults: fs2.Pipe[Task, Char, Record] = CSVParser[Task].parse

  val parser: fs2.Pipe[Task, Char, Record] =
    CSVParser.config
      .mapHeader(Map("melting temperature [F]" -> "meltingTemp", "boiling temperature [F]" -> "boilingTemp"))
      .parser[Task]
      .parse

  val rendererWithDefaults: fs2.Pipe[Task, Record, Char] = CSVRenderer[Task].render

  val renderer: fs2.Pipe[Task, Record, Char] =
    CSVRenderer.config
      .fieldDelimiter(';')
